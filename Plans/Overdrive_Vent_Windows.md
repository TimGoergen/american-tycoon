...
### Still open — device feel only (Tim has played the current build)

1. **Gesture tolerances** — `vent_gap_max` 0.40 / `vent_tap_max` 0.25 are sloppy-thumb-generous
   first cuts; only thumbs on glass can calibrate them. The distinction that matters: a missed
   beat reading as *the game didn't register a lift I made* is a tolerance bug, not difficulty.
2. **Smoothed instrument readability** — does the 1.2 s approach read now that it moves every
   frame, and is the freeze presentation legible as "this property is DOWN"?
3. **Freeze feel** — verdict 1 settles the *intent*; whether the freeze lands as tense-and-fair
   rather than punitive is still a device question. See checklist §6 sitting A.
