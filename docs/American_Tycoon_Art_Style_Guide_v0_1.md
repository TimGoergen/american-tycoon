...
**Epoch reskins (updated 2026-06-16):** staffing is now an epoch-keyed *tier track* (GDD §6.1), so each property's staffer is **reskinned per epoch** — Earth's ATM Technician becomes the Luminari *Photon Teller*, the Geth *Autonomous Teller Unit*, the Mycelium *Spore-Cash Node*, etc. Plan the asset bill accordingly: the full 52-property ladder × the 6 shipped epochs ≈ **312 cards** (GDD §12), each carrying its civilization's visual flavor over the same medallion frame. The **quiet ratio** stat pair (hire cost vs. lifetime revenue) is now a *deferred* satirical addition (GDD §6.4), not a required element of the card.

**Alien property types (added 2026-06-28; escalating ladder 2026-07-15, GDD §5.5 site 2):** the ladder now has **52** properties — the 12 Earth rows plus **40 alien property types** in an escalating cohort per epoch (**6 / 7 / 8 / 9 / 10** rungs for epochs 2–6; cohort size = tier + 4). The epoch **flagships** — **Photon Exchange** (epoch 2), **Data Foundry** (3), **Spore Bank** (4), **Prism Vault** (5), **Time Bank** (6) — are the cheapest rung of each alien cohort, no longer the *only* alien property in the epoch. Each alien property needs its own §5 hero card plus a staffer reskin for the epochs in which it's staffable, so the full staffer-card bill is ~**312 portraits (52 rungs × 6 epochs)** — which is exactly why staffer portraits moved to the procedural layered generator (GDD §6.5, §12). Until that art lands, each ships a placeholder accent color (a per-property hue, the documented §1 exception — see `PropertyConfig.accent_color`) and the lettered-medallion fallback. Prompt themes: light/energy (Photon Exchange), machine/data (Data Foundry), fungal/spreading (Spore Bank), crystal/faceted (Prism Vault), time/clockwork (Time Bank).
