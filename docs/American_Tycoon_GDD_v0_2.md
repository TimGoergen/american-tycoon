...
### 5.2 Rush Momentum → Overheat → Cruise Control (shipped; merged to main 2026-07-16)

Layer 2 grew a companion system: sustained rushing has its own meter on top of the per-tap value. It shipped in three dated steps, each layered on the last:
...
All knobs (the `rush_momentum_*` heat build/bleed rates, band edges, bonuses, locked-drain rate, re-arm delay, cruise bonus) live in Balance Tuning; `rush_momentum_max_bonus` is superseded. Remaining open: per-tier haptic/sound polish (device pass), and whether the overdrive duty-cycle edge (~+10 points over cruise) is thick enough to tempt.
...
2. **Epoch change / First Contact — reward = a NEW PROPERTY TYPE; minigame = upside-only bonus (REDESIGNED 2026-07-01 — supersedes the starting-units model).** Theme: *negotiating the alien trade deal*; winning opens a genuinely new *kind* of business in the bigger market.
   ...
   - **The cohort carries the epoch's income leap — not staff.** As shipped (continuous-ladder rework 2026-07-12, re-gridded for the escalating cohorts 2026-07-15): each epoch's cohort spans exactly **×16807** (= 7⁵, matching the per-epoch threshold growth) — rung k of an N-rung cohort costs `flagship_cost × (16807^(1/N))^k`, so the per-rung ratio shrinks as cohorts grow (×5.06 at epoch 2 down to ×2.64 at epoch 6). Income holds `income = cost × 0.01824` on a **60 s cycle** across every alien rung (verified against the shipped `.tres` files by the sim's pattern check). The flagship anchor (rung 0) is priced off the player's epoch-entry WEALTH — ~10% of the previous threshold, a real save-up — with income anchored separately so each flagship is a clear **~3× step up** over a staffed Earth property (Tim 2026-07-11/12). *(Supersedes the earlier threshold-anchored `economy_step 60 / DRIFT 0.95` formula of Phase 3, 2026-07-11 — see `Plans/Epoch_Depth_Pass.md` for that history; sim-verified via the pacing MEASUREMENT + PLAYOUT + step-up CHECK in `sim/Sim.gd` and the cohort pattern check in `sim/EpochTest.gd`.)*
