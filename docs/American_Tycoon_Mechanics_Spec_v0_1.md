...
- **Epoch pacing — the law (reworked 2026-06-27, GDD §6.2):** time to clear an epoch ≈ (dollars to earn) ÷ (income/sec), and dollars-to-earn scales with `economy_scale` while income scales with `staff_income_multiplier`, so the **per-epoch duration ratio = economy_step ÷ staff_step**. The original v1 ladder (economy ×1,000/epoch, staff ×~17/epoch) made each epoch ~60× *longer* than the last — a guaranteed stall by epoch 2. The ladder is now matched geometric with staff stepping faster: `economy_scale = 30^(tier−1)`, `staff_income_multiplier = 40^(tier−1)`, so each epoch arrives ~0.75× the prior (accelerates). Verified by `sim/Sim.gd._run_epoch_timing_study()`, which measures the live ladder's projected per-epoch durations on every run. **Caveat:** this governs *pacing only* — `staff_cost_fraction` (alien-staff affordability, always 0.1% of the epoch economy regardless of the steps) and the "something to do during an epoch" engagement layer are separate open items.
